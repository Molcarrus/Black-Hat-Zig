# Put payload in .rdata section

## TL;DR

[See the code example](https://github.com/CX330Blake/Black-Hat-Zig/tree/main/Payload-Placement/dot_rdata_section)

## Code Explanation

In this chapter, I will show you how to store out payload into .rdata section.

```zig title="main.zig"
const std = @import("std");
const print = std.debug.print;

// msfvenom calc shellcode
// msfvenom -p windows/x64/exec CMD=calc.exe -f c
// .rdata saved payload
//
// use `const` to put it in .rdata (read-only data)
const rdata_section_payload = [_]u8{
    ,
};

// Helper function to wait for Enter key
fn waitForEnter() void {
    var buffer: [256]u8 = undefined;
    _ = std.io.getStdIn().reader().readUntilDelimiterOrEof(buffer[0..], '\n') catch {};
}

pub fn main() !void {
    print("[i] Payload address: 0x{X} \n", .{@intFromPtr(&rdata_section_payload)});
    print("[i] Data size: {d} bytes\n", .{rdata_section_payload.len});
    print("[#] Press <Enter> To Quit ...", .{});
    waitForEnter();
}
```

Add the payload to .rdata section is easy too. The only difference between the previous one is we changed the `var` to `const`. That way, the payload will become read-only and be placed in.rdata section.
